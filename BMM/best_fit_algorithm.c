...
void *best_fit(MemoryManager *manager, size_t size)
{
    ...
    if (manager == NULL || manager->memory_pool == NULL || size == 0)
    {
        return (NULL);
    }
    if (size > manager->pool_size)
    {
    ...
    while ((char *)current_block < pool_end)
    {
        ...
        current_block = (MemoryBlock *)((char *)current_block + current_block->size + sizeof(MemoryBlock));
    }
    ...
    if (best_fit_block != NULL)
    {
        ...
        {
            MemoryBlock *new_block = (MemoryBlock *)((char *)best_fit_block + size + sizeof(MemoryBlock));

            set_block_size(new_block, min_remaining_size - sizeof(MemoryBlock));
